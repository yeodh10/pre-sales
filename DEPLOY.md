...
데모를 단일 URL로 서비스하기 위한 Vercel 배포 절차입니다.
...
4. 설정 화면에서:
   ...
   - **Production Branch**: `claude/charming-hamilton-8pj5ta`를 그대로 쓰거나,
...
| Key | Value | 필수 |
| --- | --- | --- |
| `ANTHROPIC_API_KEY` | `sk-ant-...` (console.anthropic.com 발급) | 선택 (없으면 룰 폴백) |
| `ANTHROPIC_MODEL` | `claude-sonnet-4-6` (미설정 시 기본값) | 선택 |

> 환경 변수는 **Production / Preview / Development** 환경 모두에 적용되도록 체크하세요.
> 키 없이 배포해도 동작합니다 — "솔루션 추천 받기"가 룰 기반 폴백 엔진으로 응답하며,
> 결과에 폴백 안내 배너가 표시됩니다. LLM 추천을 쓰려면 키가 필요합니다.
...
- [ ] 화면 상단/하단 "비공식 개인 기술 데모" 디스클레이머 노출
...
- 콜드 스타트: 서버리스 함수 특성상 유휴 후 첫 요청이 느릴 수 있습니다.
  시연 직전에 추천을 한 번 호출해 워밍업하면 응답이 안정적입니다.
